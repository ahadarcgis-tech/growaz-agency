...
export function CardStack({
  ...
  const [dimensions, setDimensions] = React.useState({ width: cardWidth, height: cardHeight });
  ...
  React.useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      ...
      if (entry) {
        ...
        if (containerWidth < cardWidth + 32) {
          const newWidth = Math.max(280, containerWidth - 32);
          const ratio = newWidth / cardWidth;
          const newHeight = Math.round(cardHeight * ratio);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ height: Math.max(380, dimensions.height + 80) }}
        tabIndex={0}
        onKeyDown={onKeyDown}
      >
        <div
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    ...
                    "will-change-transform select-none bg-white",
                    isActive
            ...
            {items.map((it, idx) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "h-2.5 w-2.5 rounded-none border border-border transition cursor-pointer",
                    on
  ...
  return (
    <div className="relative h-full w-full bg-white">
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-6 text-left">
        <span className="text-xs font-bold text-accent uppercase tracking-widest mb-1">
          {item.tag || "PROJECT"}
        </span>
        <div className="truncate text-xl font-bold text-white uppercase tracking-tight">
          {item.title}
        </div>
        {item.description ? (
          <div className="mt-1 line-clamp-2 text-sm text-white/80 font-normal leading-relaxed">
            {item.description}
          </div>
        ) : null}
      </div>
